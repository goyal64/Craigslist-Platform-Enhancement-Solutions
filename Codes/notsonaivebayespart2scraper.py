...
class Aud1Spider(scrapy.Spider):
    name = 'notsonaivebayespart2scraper'
    ...
    def parse(self, response):
        ...
        for listing in listings:
            ...
            yield Request(low_url, callback=self.parse_lower,
                          meta={'Title': listing_url})

        next_rel_url = response.xpath('//a[@class="button next"]'
                                      '/@href').extract_first()
        next_url = response.urljoin(next_rel_url)
        yield Request(next_url, callback=self.parse)

    def parse_lower(self, response):
        date = response.xpath('//time[contains(@class, "timeago")]'
                              '/@datetime').extract_first()[:10]
